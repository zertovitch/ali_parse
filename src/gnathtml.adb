...
procedure GNATHTML is
  ...
  exception
    when Name_Error =>
      --  Snippet not found is like an empty snippet.
      null;
  end Insert_Snippet;
        ...
        begin
          case l (x) is
            ...
            when ''' =>
              ...
              x := x + 1;
              if x > l'Last then
                null;  --  End of line -> unfinished character literal: only "'".
              else
                ...
                if x > l'Last then
                  ...
                  --  We just had a full character literal, like '?' or '''.
                  Put (html, ''');
                  x := x + 1;
                else
  ...
  begin
    ...
    Put_Line
      (html,
       "<div align=""center""><h2>" & font_var_width & "Source file : " &
       ada_name &
       "</font></h2></div><hr>");
    Put_Line (html, "<pre>" & font_fix_width);
    Scan;
    Put_Line (html, "</font></pre>");
    Close (ada_source);
    Put_Line
      (html,
       "<br>" & font_var_width &
       "Web view of Ada source code generated by GNATHTML, project: <a target=""_blank"" href=" &
       ALI_Parse.web1 &
       ">ALI_Parse</a> version " &
       ALI_Parse.version &
       ".</font>");
